...
| [17-batch-system.md](./17-batch-system.md) | batch 拆分、连续批处理 |
...
### 4.2 Decode 循环（详见 [15-decode-graph-reuse.md](./15-decode-graph-reuse.md)）
...
### 4.4 Memory 工厂（详见 [16-kv-cache-memory.md](./16-kv-cache-memory.md)）
...
- **图复用**: `llm_graph_result::can_reuse()` 避免重复建图（见 [15-decode-graph-reuse.md](./15-decode-graph-reuse.md)）
- **Flash Attention**: `-fa on` 启用，减少 KV cache 内存
- **KV 量化**: `--cache-type-k q8_0` 降低 cache 内存
- **Batch 拆分**: `n_ubatch` 控制单次计算 token 数（见 [17-batch-system.md](./17-batch-system.md)）
- **Thread 池**: `n_threads` / `n_threads_batch` 控制并行度
- **mmap**: 默认启用，加速模型加载（见 [14-model-loader-deep-dive.md](./14-model-loader-deep-dive.md)）

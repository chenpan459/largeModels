...
## 6. build_arch_graph 实现模式

每个模型文件遵循统一模式（以 `llama_model_llama` 为例）：
...
| 基类 | 用途 |
|------|------|
| `llm_graph_context` | 通用 Transformer 层（定义在 llama-graph.h） |
| `llm_build_mamba_base` | Mamba/Mamba2 SSM 层 |
| `llm_build_delta_net_base` | Gated DeltaNet 层 |
| `llm_build_rwkv6_base` | RWKV6 time/channel mix |
| `llm_build_rwkv7_base` | RWKV7 变体 |

### 6.2 llama_model_base 提供的通用能力

| 方法 | 说明 |
|------|------|
| `create_tensor()` | 包装 loader.create_tensor |
| `create_tensor_qkv/gate_up_exps` | 合并权重 fallback |
| `load_stats/hparams/vocab/tensors` | 通用加载流程 |
| `LLAMA_LOAD_LOCALS` 宏 | load_tensors 局部变量 |

### 6.3 上层 build_graph 包装

`llama_model::build_graph()` 在 arch graph 之上统一追加：

```
build_arch_graph(params)     # 架构特定

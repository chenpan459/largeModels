...
详见 [14-V0-V1迁移与自动选择.md](./14-V0-V1迁移与自动选择.md)。
...
1. 本文 → [02-架构与推理数据流.md](./02-架构与推理数据流.md) → [09-Entrypoints与OpenAI-API.md](./09-Entrypoints与OpenAI-API.md)
2. 机制深入：[05-kv-cache](./05-KV-Cache与PagedAttention.md) → [04-scheduler](./04-Scheduler与Continuous-Batching.md) → [03-v1-engine](./03-V1-Engine与Executor.md)
3. V0/V1 差异：[14-V0-V1迁移与自动选择.md](./14-V0-V1迁移与自动选择.md)
4. 模型与内核：[06-model-executor](./06-Model-Executor与GPUModelRunner.md) → [07-attention-backends](./07-Attention后端.md) → [13-sampling](./13-采样与结构化输出.md)
